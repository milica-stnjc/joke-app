...
with c1:
    ...
    if cat_button:
        ...
        st.image(cat_image, width=300, caption= "THIS IS MY CAT IMAGE")

with c2:
    dog_button = st.button("Click here for a dog image")
    if dog_button:
        dog_image_url = get_dog_image_url()
# if the url ends with mp4, keep fetching new url
        while dog_image_url[-4:] == '.mp4':
            dog_image_url = get_dog_image_url()

        st.image(dog_image_url, width=300, caption= "THIS IS MY DOG IMAGE")
